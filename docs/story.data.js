...
},
{
  ...
  res:"The Voynich's h2 is about 2.4 bits against 3.38 to 3.61 for the reference languages (eighteen works, three per language): its glyphs are far more predictable than any of the real scripts measured. Two probes chosen to close that gap from above widen it instead: Hawaiian, whose twelve-letter alphabet makes it the natural script most likely to score low, reaches 2.85 on the 1868 Hawaiian Bible, still 0.45 bits above the Voynich; and medieval Latin transcribed with its scribal abbreviations preserved scores 3.9 to 4.4, above the printed-prose band, because abbreviation removes exactly the predictable letters. The Voynich's word lengths are unusually uniform too (standard deviation 1.82 vs 2.4 to 2.9). These two facts are what the rest of the project has to explain.",
  next:"The manuscript is largely a herbal. Could the low entropy be a specialised-vocabulary effect?",
  fig:{type:"bars", unit:"h2 (bits) · lower = more predictable", baseline:3.5, band:[3.38,3.61], data:[
    ["VOYNICH",2.40,true],["HAWAIIAN",2.85,false],["ENGLISH",3.60,false],["LATIN",3.45,false],["GERMAN",3.43,false],["HEBREW",3.89,false]]},
...
<p>The Voynich script reads as if almost every position were closer to the <span class="hl">q</span> situation: at each step, the current glyph strongly constrains the next. No natural script we measured behaves this way, which is why this number, together with the too-uniform word lengths, is what the rest of the investigation has to explain.</p>`},
  {t:"How hard we looked for a natural script this low", h:
`<p>A fair objection to this chart: six reference languages, one book each, is a narrow basis for the word "anomaly". So we widened it. Three works per language, eighteen in all, span 3.38 to 3.61 bits; the spread within a language is real but small, and the Voynich sits a full bit below the lowest of the eighteen.</p>
<p>Then we went looking for trouble. Hawaiian spells its entire vocabulary with about twelve letters, so each letter is common and its continuations are constrained; of the scripts we could measure it is the one built to score low. The 1868 Hawaiian Bible comes in at <span class="hl">2.85 bits</span>: well below every European work, the closest natural point to the Voynich we have found, and still 0.45 bits short of it.</p>
<p>The hardest case is the one a fifteenth-century reader would name: working scribal Latin, written dense with abbreviation signs. Nearly every modern edition silently expands those abbreviations back into full words, which would hide any effect. We found a corpus that does not: 21 medieval Latin manuscripts transcribed exactly as written, signs and all (CREMMA-Medieval-LAT). Measured that way, the entropy goes <span class="hl">up</span>, to about 4.2 bits, not down. Abbreviation deletes precisely the letters a reader could have predicted, and what remains is less predictable per glyph. The register most often proposed as the Voynich's natural neighbour turns out to sit on the far side of the band.</p>`}],
...
},
{
  ...
  test:"For each word, found the minimum edit distance to the previous few words, compared against a shuffle of the same tokens (identical vocabulary, sequential copying destroyed). Tracked three signatures: below-shuffle nearest distance, an excess of near-identical neighbours, and a locality gradient. Added two hostile controls built to fire the signatures by construction: a Latin declension table and a litany.",
  verdict:"v-yes", verdictText:"LARGEST EFFECTS",
  res:"All three signatures exceed their null distributions by wide margins: nearest-distance excess z≈78, copy spike z≈50, locality gradient z≈23, against 200-shuffle null distributions. No language shows the ensemble. Hebrew, the closest runner-up, matches the exact-repeat spike (for a known linguistic reason: a consonant-only script repeats short forms constantly) but has only half the locality gradient and none of the entropy or word-length anomalies; the prose languages' gradients are negative. The Voynich's copying is specifically local. Two genre controls confirm the signatures alone are not the fingerprint: a Latin declension-table text fires all three at roughly ten times the Voynich's magnitude while keeping Latin-level entropy and 88% unique word forms, and a litany fires the exact-repeat spike while carrying thirty times the Voynich's word-order information. No measured text combines Voynich-level copy signatures with the low-entropy, near-zero-order profile. A single process accounts for the slot grammar, the near-zero word order and the local repetition: copy a nearby word and mutate it.",
...
<p>This plate's statistics test that impression manuscript-wide: measure how close each word is to its recent predecessors, then ask whether shuffling the same words away destroys the closeness. It does, massively, and only in the Voynich. The shuffle is the essential control: the slot grammar (Plate 6) makes ALL Voynich words look alike, so raw similarity proves nothing; only similarity beyond the shuffled level counts as copying.</p>`},
  {t:"Trying to fool the test with a grammar book and a litany", h:
`<p>Before trusting these signatures we tried to break them. Two kinds of real text produce near-copies of recent words by construction. A grammar book's paradigm tables recite <span class="hl">rosa, rosae, rosae, rosam</span>: the same stem with the ending varied, word after word. A litany alternates a changing invocation with a fixed response (<span class="hl">Sancte Petre, ora pro nobis; Sancte Paule, ora pro nobis</span>), so the same phrase returns every few words. If the copy test cannot tell these from the manuscript, its verdict means little.</p>
<p>Both fired, exactly as predicted before running, and neither resembles the Voynich in anything else. The paradigm table (built from real Latin stems and endings, 11,486 words) fires every signature about ten times harder than the manuscript, and the mismatch shows everywhere else: 88 of every 100 of its words appear exactly once, its frequency curve is nearly flat where the Voynich's is steep, and its letter entropy is ordinary Latin. The litany fires the exact-repeat spike through its fixed response but nothing further: its words follow one another in a fixed order, giving it about thirty times the Voynich's word-order information, and its repeats do not favour the recent past the way copying does.</p>
<p>The manuscript holds a combination neither control has: copying strong enough to dominate the statistics yet far below table-recitation level, letter entropy at the bottom of the scale, and word order carrying almost nothing. The test separates a grammar book and a litany from the Voynich without difficulty, and that is what makes its verdict on the manuscript worth having.</p>`},
  {t:"Could the copying just be reduplication?", h:
`<p>A question worth taking seriously: plenty of real languages repeat words on purpose. Malay doubles a noun to make it plural (orang orang, "people"); liturgical and poetic texts double for emphasis; and the manuscript has famous runs like <span class="hl">daiin daiin</span> and whole families like <span class="hl">qokedy, qokeedy, qokeddy</span>. If reduplication is something languages genuinely do, does the copy test read an ordinary linguistic device as evidence of mechanical generation?</p>
<p>The shuffle control settles most of it. The test never scores a word for resembling other words in the vocabulary; it scores similarity to <span class="hl">recent</span> words beyond what a reshuffle of the same tokens produces. A language whose word-stock is full of doubled or related forms keeps that word-stock in the shuffle, so it scores zero by construction. What a shuffle cannot explain away is placement: similar forms sitting close together in the stream, with the similarity fading the further back you look. That fading is the signature of copying, and no fixed vocabulary produces it, however reduplicated.</p>
<p>The manuscript's own numbers then point away from deliberate doubling. Exact adjacent repeats, the reduplication signature proper, are rare: 0.85% of tokens, an excess of just +0.005 over shuffle. Adjacent pairs one edit apart (qokedy followed by qokeedy) run at 3.8%, an excess four times larger. Deliberate doubling predicts the opposite ratio, since the whole point of reduplication is to repeat the word exactly. What the text actually contains is chains of variants, each a small mutation of a neighbour, and collapsing every exact doublet and re-measuring leaves all the headline statistics essentially unchanged. The grammar-book and litany controls in the block above were the stress test for texts that near-repeat by construction, and both are easy to tell apart from the manuscript.</p>
<p>Words like <span class="hl">aiiv</span> and <span class="hl">aiin</span> raise a different form of the question: repeated strokes inside a word, which could be one glyph mis-split into several by the transcription. That was tested separately: re-running the whole analysis with the minim runs regrouped as single glyphs leaves the entropy anomaly intact, still about half a bit below the language band. One gap we will own: no corpus from a heavily reduplicating language family (Austronesian, say) has been run through the harness. The grammar-book and litany controls cover the mechanism of built-in near-repeats, but that exact comparison remains open.</p>`}],
...
},
{
  ...
  res:"The four-parameter unified generator reproduces the whole fingerprint (combined error 0.179) better than any specialist (0.274). Scored only on the metrics no model was tuned to, the stricter comparison, its margin widens: 0.219 against 0.361. The Zipf slope, type count and Heaps growth emerge unfitted (generator Zipf -0.87 vs Voynich -0.89). Nothing in the internal statistics requires meaning. Sufficiency is not necessity: a highly formulaic real text is not excluded.",
...
},
{
  ...
  res:"The low h2, narrow length and self-citation signal all hold. The self-citation signal is absent from real prose, a Latin word list and enciphered Latin; the genre texts that do fire individual copy signatures (a declension table, a litany; Plate 8) match nothing else in the profile. Several figures were revised downward: '75%' to about a third; a claimed stable-collocation signal was retracted at about 1σ; the '5% dialect' component measured zero; a topical-clustering signal halved. All corrections reduced the strength of previously published claims; most favoured the mechanical reading, while the long-range correction favoured the content reading.",
